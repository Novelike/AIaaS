...
async function sendDiscordWebhook({ action, username, email, userId }) {
    ...
    const embed = {
        title: isInsert ? "👤 새 사용자 등록" : "❌ 사용자 삭제",
        color: isInsert ? 0x00cc99 : 0xff5555, // 초록 / 빨강
        fields: [
            { name: "🆔 사용자 ID", value: String(userId), inline: true },
...
app.post("/api/users", async (req, res) => {
    try {
        ...
        if (!username || !email) {
            return res
                .status(400)
                .json({ error: "사용자명과 이메일은 필수 입력 항목입니다." });
        }
        ...
        await sendDiscordWebhook({
            ...
            username,
            email,
            userId: result.insertId, // 또는 삽입된 사용자 ID
        });
...
app.delete("/api/users/:id", async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res
                .status(404)
                .json({ error: "해당 ID를 가진 사용자가 존재하지 않습니다." });
        }
